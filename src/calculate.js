...
export function formatTimestamp(seconds, asString = false) {
...
export function calculateDeltas(data, yearCache) {
    const {owner_id, event, options} = data;
    return new Promise((resolve, reject) => {
        const members = Object.values(data.members);
        const member = members.find(member => member.id == owner_id);
        const completion_day_level = member.completion_day_level
        Object.keys(completion_day_level).forEach(dayNum => {
            const puzzleDropDate = new Date(event, 11, dayNum);
        ...
        resolve({completion_day_level, options});
    });
}
